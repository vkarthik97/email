...
public class UserRepositoryImpl implements UserRepository {
	...
	public RegisterBean authenticate(LoginBean login) {
		...
		try {
			...
			if (rs.next()) {
				System.out.println("if"+login.getEmail());
	...
	public boolean validate(ForgetBean forget) {
		String sql = "select *from users where email=? and qn=?";
		Connection conn = null;
		try {
			conn = getConnection();
	...
	public boolean changePassword(LoginBean login) {
		...
		try {
			conn = getConnection();
			PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.setString(1, login.getPass());
